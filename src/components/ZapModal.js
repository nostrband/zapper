...
import { copy, formatSats } from '../utils';
...
function ZapModal({ isOpen, onClose, currentZap, zaps, onDone }) {
  ...
  return (
    ...
    >
      <Modal.Header>
        <Modal.Title>Sending{zaps.length > 1 && ` ${currentZap.index + 1}/${zaps.length}`}...</Modal.Title>
      </Modal.Header>
      <Modal.Body className="d-flex flex-column">
        <Profile event={currentZap} />
        <div className="mt-3"><h5>{formatSats(currentZap.amount)} sats</h5></div>
        {currentZap.comment && (
        ...
        {currentZap.status === 'waiting' && (
          <>
            ...
            <InputGroup>
              ...
              />
              <Button variant="outline-secondary" id="copy-invoice" onClick={() => copy(currentZap.invoice)}>
                <Clipboard />
              </Button>
              <Button variant="outline-secondary" onClick={() => setShowQR(!showQR)}>
                <QrCodeScan />
              </Button>
            </InputGroup>

            <center>
              <canvas id='canvas' className={!showQR ? 'd-none' : ''}></canvas>
            </center>
