...
function ZapForm() {
	...
	useEffect(() => {
		const update = async () => {
			...
			const totalWeight = targetPubkeyWeights.reduce(
				(total, pw) => (total += pw.weight),
				0,
			)
		...
		<div>
			...
			<div>
				...
				>
					<Modal.Header closeButton>
						<Modal.Title>
							Auto zapping in {autoSendTimer} seconds...
						</Modal.Title>
					</Modal.Header>
				...
				>
					<Modal.Body>
						...
						<center className='mt-3'>
							<CheckCircle
								color={hasErrors ? 'yellow' : 'lime'}
								size={96}
							/>
						</center>
						{hasErrors && (
							<p className='mt-4'>
								You can view logs and retry with the failed
								recipients.
							</p>
						)}
					</Modal.Body>
